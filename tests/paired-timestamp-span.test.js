...
test("paired timing brackets actual compute passes and rejects incomplete spans",()=>{
  ...
  native.clearBuffer=(...args)=>copies.push(args);
  encoder.clearBuffer("reset",0,16);
  ...
  assert.deepEqual(copies,[["reset",0,16],[1,2,3,4,5]]);assert.equal(encoder.finish(),"commands");
